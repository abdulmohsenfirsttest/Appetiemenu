  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div style={{ fontWeight: 700, fontSize: 16, color: 'var(--admin-text)' }}>Sales Reports</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        <div style={{ background: 'var(--admin-card)', border: '1px solid var(--admin-border)', borderRadius: 12, padding: 20 }}>
          <div style={{ fontWeight: 700, marginBottom: 16, color: 'var(--admin-text)' }}>Orders per Day</div>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={sales}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9"/>
              <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#94a3b8' }}/>
      ...
      {top.length > 0 && (
        <div style={{ background: 'var(--admin-card)', border: '1px solid var(--admin-border)', borderRadius: 12, padding: 20 }}>
          <div style={{ fontWeight: 700, marginBottom: 16, color: 'var(--admin-text)' }}>Top Products by Revenue</div>
          {top.map((p, i) => (
            <div key={p.product_name} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                  <span style={{ fontWeight: 600, fontSize: 13, color: 'var(--admin-text)' }}>{p.product_name}</span>
